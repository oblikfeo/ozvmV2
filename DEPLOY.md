...
Вход проверяет сервер, basic auth на nginx больше нет. Завести
администратора или сменить пароль:

```bash
docker exec -w /var/www/api.ozvm.ru php php artisan retail:admin <логин> <пароль>
```

Смена пароля отзывает все выданные токены — открытые сессии закрываются.

## Бэкенд

Данные лежат в базе основного проекта. В Laravel добавлены только новые
сущности, существующие таблицы и код магазина не тронуты:

- миграции `2026_09_25_*` — таблицы `promotions`, `landing_contents`, `admins`
- контроллеры `app/Http/Controllers/Api/v1/Retail/`
- модели `Promotion`, `LandingContent`, `Admin`
- guard `admin` в `config/auth.php` и ветка `v1/retail` в `routes/api.php`
  (у обоих файлов рядом лежит бэкап `.bak-<дата>`)
...
# демонстрационные акции, только в пустую таблицу
docker exec -w /var/www/api.ozvm.ru php php artisan retail:seed-promotions
```

Загруженные картинки акций кладутся файлами в `public/images/promotions/`
на API и отдаются его адресом. Картинки из встроенной галереи хранятся
относительным путём и лежат на самом сайте.
...
Токен администратора лежит в `localStorage` под ключом `RETAIL_ADMIN_TOKEN`.
Когда появится вход покупателя, ключи нужно развести: права у них разные.
